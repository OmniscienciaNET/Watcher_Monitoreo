...
function setCookie(nombre, valor) {
    ...
    document.cookie = "CC" + nombre + "=" + valor + ";expires=" + now.toUTCString() + ";path=/";
}
...
function BuscarCierre() {
    document.getElementById("btnSearch").onclick = function () {
        ...
        var url = "../Modulo/GrillaControlCierre?DstbCod=" + dstb + "&AbndCod=" + abnd + "&InOutSide=" + inOutSide + "&Ubigeo=" + dpto + "&TipoAbonado=" + tipoAbonado + "&t=" + Math.random();
        enviarServidor(url, GrillaCierre);
    }
}
    ...
    document.getElementById("exportarExcel").onclick = function () {
        if (matriz.length != 0) {
            ...
            if (navigator.appVersion.toString().indexOf('.NET') > 0)
                window.navigator.msSaveBlob(blob, "ControlCierre.xls");
            else {
                this.download = "ControlCierre.xls";
...
function crearMatrizCierre(lista) {
...
function mostrarPaginaCierre(indicePagina) {
    ...
    var tbCierre = document.getElementById("tbRegistrosCierre");
    tbCierre.innerHTML = contenido;
}

function crearPaginasBloqueCierre() {
    var contenido = "";
    var existeBloque = (matriz.length > (registrosPagina * paginasBloque));
    if (existeBloque) {
        contenido += "<a href='#' onclick='paginarCierre(-1);' title='Ir al primer grupo de páginas'>&lt;&lt;</a>";
        contenido += "<a href='#' onclick='paginarCierre(-2);' title='Ir al anterior grupo de páginas'>&lt;</a>";
    }
    ...
    for (i = 0; i < paginasBloque; i++) {
        ...
        if (n <= indiceUltimaPagina) {
            contenido += "<a href='#' onclick='paginarCierre(" + n + ");'  title='Ir a la pagina " + (n + 1).toString() + "' id='a" + n.toString() + "' class='pagina' >" + (n + 1).toString() + "</a>";
        }
    }
    if (existeBloque) {
        contenido += "<a href='#' onclick='paginarCierre(-3);' title='Ir al siguiente grupo de páginas'>&gt;</a>";
        contenido += "<a href='#' onclick='paginarCierre(-4);' title='Ir al último grupo de páginas'>&gt;&gt;</a>";
    }
    var tdPagina = document.getElementById("liPaginaCierre");
    tdPagina.innerHTML = contenido;
}

function paginarCierre(indicePagina) {
    if (indicePagina < 0) {
        ...
        mostrarPaginaCierre(indicePagina);
        crearPaginasBloqueCierre();
    }
    else {
...
function seleccionarPaginaActualCierre() {
