...
function setCookie(nombre, valor) {
    ...
    document.cookie = "CS" + nombre + "=" + valor + ";expires=" + now.toUTCString() + ";path=/";
}
...
function BuscarSenal() {
    document.getElementById("btnSearch").onclick = function () {
        ...
        var url = "../Modulo/GrillaControlSenal?DstbCod=" + dstb + "&AbndCod=" + abnd + "&Time=" + time + "&InOutSide=" + inOutSide + "&Ubigeo=" + dpto + "&TipoAbonado=" + tipoAbonado + "&t=" + Math.random();
        enviarServidor(url, GrillaSenal);
    }
}
    ...
    document.getElementById("exportarExcel").onclick = function () {
        if (matriz.length != 0) {
            ...
            if (navigator.appVersion.toString().indexOf('.NET') > 0)
                window.navigator.msSaveBlob(blob, "ControlSenal.xls");
            else {
                this.download = "ControlSenal.xls";
...
function GrillaSenal(rpta) {
    if (rpta != "") {
        indiceActualBloque = 0;
        var divPaginacion = document.getElementById("divPaginacionSenal");
        divPaginacion.style.display = "inline";
        var lista = rpta.split("_");
        matriz = [];
        crearMatrizSenal(lista);
        mostrarPaginaSenal(0);
        crearPaginasBloqueSenal();
        seleccionarPaginaActualSenal();
    }
    else {
        document.getElementById("tbRegistrosSenal").innerHTML = "<tr style='border-top: 1px solid #222;border-bottom: 1px solid #222'><td colspan='5' style='width:90px;'>No se encontraron registros</td></tr>";
        document.getElementById("divPaginacionSenal").style.display = "none";
        document.getElementById("liPaginaSenal").innerHTML = "";
    }
}

function crearMatrizSenal(lista) {
...
function mostrarPaginaSenal(indicePagina) {
    ...
    var tbSenal = document.getElementById("tbRegistrosSenal");
    tbSenal.innerHTML = contenido;
}

function crearPaginasBloqueSenal() {
    var contenido = "";
    var existeBloque = (matriz.length > (registrosPagina * paginasBloque));
    if (existeBloque) {
        contenido += "<a href='#' onclick='paginarSenal(-1);' title='Ir al primer grupo de páginas'>&lt;&lt;</a>";
        contenido += "<a href='#' onclick='paginarSenal(-2);' title='Ir al anterior grupo de páginas'>&lt;</a>";
    }
    ...
    for (i = 0; i < paginasBloque; i++) {
        ...
        if (n <= indiceUltimaPagina) {
            contenido += "<a href='#' onclick='paginarSenal(" + n + ");'  title='Ir a la pagina " + (n + 1).toString() + "' id='a" + n.toString() + "' class='pagina' >" + (n + 1).toString() + "</a>";
        }
    }
    if (existeBloque) {
        contenido += "<a href='#' onclick='paginarSenal(-3);' title='Ir al siguiente grupo de páginas'>&gt;</a>";
        contenido += "<a href='#' onclick='paginarSenal(-4);' title='Ir al último grupo de páginas'>&gt;&gt;</a>";
    }
    var tdPagina = document.getElementById("liPaginaSenal");
    tdPagina.innerHTML = contenido;
}

function paginarSenal(indicePagina) {
    if (indicePagina < 0) {
        ...
        mostrarPaginaSenal(indicePagina);
        crearPaginasBloqueSenal();
    }
    else {
...
function seleccionarPaginaActualSenal() {
