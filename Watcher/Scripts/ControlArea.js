...
function setCookie(nombre, valor) {
    ...
    document.cookie = "CA" + nombre + "=" + valor + ";expires=" + now.toUTCString() + ";path=/";
}
...
function BuscarArea() {
    document.getElementById("btnSearch").onclick = function () {
        if (document.getElementById("txtCondicPrimCod").value.trim() == "" && document.getElementById("txtCondicUltCod").value.trim() != "") {
            ...
            var url = "../Modulo/GrillaControlArea/?DstbCod=" + dstb + "&AbndCod=" + abnd + "&CondPrimCod=" + condP + "&CondUltCod=" + condU + "&Zona=" + zona + "&Time=" + opc + "&DatosP=" + pasado + "&InOutSide=" + inOutSide + "&Ubigeo=" + dpto + "&TipoAbonado=" + tipoAbonado + "&t=" + Math.random();
...
function Exportar() {
    ...
    document.getElementById("exportarExcel").onclick = function () {
        if (matriz.length != 0) {
            ...
            excelExportar+=crearReporteExcel();
            var blob = new Blob([excelExportar], { type: 'application/vnd.ms-excel' });
            if (navigator.appVersion.toString().indexOf('.NET') > 0)
                window.navigator.msSaveBlob(blob, "ControlArea.xls");
            else {
                this.download = "ControlArea.xls";
